...
def _wandb_login_from_dotenv(dotenv_path: str) -> None:
    """
    Loads wandb_api_key from .env in the same directory as this file, then logs into W&B.
    Also sets WANDB_API_KEY in the environment for downstream tooling.
    """
    api_key = os.environ.get("WANDB_API_KEY") or os.environ.get("wandb_api_key")

    if not api_key:
        api_key = _read_env_key(dotenv_path, "wandb_api_key")

    if not api_key:
        raise RuntimeError(
            f"W&B API key not found. Expected either env var WANDB_API_KEY / wandb_api_key "
            f"or a line `wandb_api_key = ...` in: {dotenv_path}"
        )

    os.environ["WANDB_API_KEY"] = api_key  # standard env var name W&B expects
...
def main(config: Configuration):
    ...
    # --- W&B login from .env (same dir as this file) ---
    dotenv_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), ".env")
    _wandb_login_from_dotenv(dotenv_path)
    ...
    config.logdir = os.path.join(
        C.RESULTS_DIR, wandb_logger.experiment.project, wandb_logger.experiment.id
    )
    ...
    dino_trfm = transforms.Compose(
        [
            ...
            MultiCrop(
                config.mc_spec,
                per_crop_transform=transforms.Compose(
                    [
    ...
    if config.label_noise_ratio > 0 and config.logit_noise_temp > 0:
        ...
        dino_train_set = LabelNoiseWrapper(
            dino_train_set, config.n_classes, config.label_noise_ratio, config.resample_noise
        )
        ...
        dino_train_set = LogitNoiseWrapper(
            dino_train_set, config.n_classes, config.logit_noise_temp, config.resample_noise
        )
        dino_valid_set = LogitNoiseWrapper(
            dino_valid_set, config.n_classes, config.logit_noise_temp, config.resample_noise
        )
    ...
    sampler = RandomSampler(
        dino_train_set, num_samples=config.samples_per_epoch, generator=generator
    )
    dino_train_dl = DataLoader(
        dataset=dino_train_set, batch_size=config.bs_train, sampler=sampler, **dl_args
    )
    ...
    print("Created encoder and head:")
    ...
    callbacks = [
        ...
        AccuracyTracker(
            n_classes=config.out_dim,  # pseudo label accuracy
    ...
    if len(config.save_features) > 0:
        config.save_features = (
            ["embeddings", "projections", "logits"] if "all" in config.save_features else config.save_features
        )
    ...
    if "none" not in config.save_ckpt:
        ...
        if "probe_student" in config.save_ckpt and config.validation_freq != 0:
            ckpt_callbacks += [
                ModelCheckpoint(
                    ...
                    mode="max",
                    save_last=False,  # checked each epoch
                    filename="epoch={epoch}-probe_student={probe/student:.3f}",
        ...
        if "loss_max" in config.save_ckpt:
            ckpt_callbacks += [
                ModelCheckpoint(
                    ...
                    filename="epoch={epoch}-step={step}-loss_max={train/loss:.1e}",
                    auto_insert_metric_name=False,
                )
            ]
        if "rank_min" in config.save_ckpt:
            ckpt_callbacks += [
                ModelCheckpoint(
                    dirpath=config.logdir,
                    monitor="train/feat/embed/s_x.rank()",
                    mode="min",
                    save_last=False,
                    every_n_train_steps=1,
                    filename="epoch={epoch}-step={step}-rank_min={train/feat/embed/s_x.rank()}",
                    auto_insert_metric_name=False,
                )
            ]
    ...
    trainer.fit(model=dino, train_dataloaders=dino_train_dl, val_dataloaders=dino_valid_dl)
    trainer.validate(model=dino, dataloaders=dino_valid_dl)
